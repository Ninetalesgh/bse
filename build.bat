@echo off
IF NOT DEFINED DevEnvDir call misc/shell.bat
...
IF NOT "%~0"=="" (
  ...
  IF "%~0"=="-app" (
    IF NOT "%~1"=="" (
      set app_path="%~1"
      IF x%out_file%==x set out_file=%~n1
      SHIFT
    )
    goto loop_parse_parameters
  )
...
IF NOT x%app_path%==x set compiler_options=%compiler_options% /DBSE_BUILD_APP_PATH=%app_path%
...
@REM ------------------------------------------------------------------------
  ...
  cl /LD %code_path%\core\bse_core.cpp %compiler_options_debug% /Fe:bse_core.dll /Fmbse_core.map %linker_options% %app_exports% /PDB:bse_core_temp_%random%.pdb 
